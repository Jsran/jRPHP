...
class Folder
{
	...
	static function create($structure, $mode = 0777, $force = false)
	{
		...
		if (is_file($structure))
		{
			...
			{				
				//之前为---->self::$_errormsgs[] = sprintf('%s is a file', $dir);
	...
	static function move($source, $target)
	{
		...
		if(!@rmdir($source)) throw new my_exception("can not rmdir $source");
		return true;
	}
